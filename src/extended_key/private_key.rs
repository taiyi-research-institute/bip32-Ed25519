...
/// Extended private curve25519 EdDSA signing key.
#[cfg(feature = "ed25519")]
pub type XPrv = ExtendedPrivateKey<Scalar>;

/// Extended private keys derived using BIP32.
///
/// Generic around a [`PrivateKey`] type. When the `ed25519` feature of this
/// crate is enabled, the [`XPrv`] type provides a convenient alias for
/// extended EdDSA/curve25519 private keys.
#[derive(Clone)]
    ...
    K: PrivateKey,
{
    ...
    pub fn to_extended_key(&self, prefix: Prefix) -> ExtendedKey {
        ...
        key_bytes[0..].copy_from_slice(&self.to_bytes());
    ...
    K: PrivateKey,
{
    ...
    fn try_from(extended_key: ExtendedKey) -> Result<ExtendedPrivateKey<K>> {
        ...
        if extended_key.prefix.is_private() {
            Ok({
                let mut tmp = [0u8; 32];
                // tmp.copy_from_slice(&extended_key.key_bytes[1..]);
                tmp.copy_from_slice(&extended_key.key_bytes);
                ExtendedPrivateKey {
                    private_key: PrivateKey::from_bytes(&tmp),
